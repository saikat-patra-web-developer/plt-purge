    ...
    <div className="overflow-hidden rounded-2xl border border-slate-700 bg-slate-950 text-slate-200">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700 bg-slate-900 px-4 py-3">
        ...
        <div className="flex items-center gap-1">
          <button type="button" aria-label="Toggle grid" onClick={() => setGrid(!grid)} className={`rounded-lg p-2 hover:bg-slate-700 ${grid ? 'bg-slate-800 text-teal-300' : ''}`}><Grid2X2 size={15} /></button>
          <button type="button" aria-label="Zoom out" disabled={zoom <= 1} onClick={() => setZoom(Math.max(1, zoom - 0.5))} className="rounded-lg p-2 hover:bg-slate-700 disabled:opacity-30"><Minus size={15} /></button>
          <span className="w-10 text-center text-[11px]">{zoom * 100}%</span>
          <button type="button" aria-label="Zoom in" disabled={zoom >= 3} onClick={() => setZoom(Math.min(3, zoom + 0.5))} className="rounded-lg p-2 hover:bg-slate-700 disabled:opacity-30"><Plus size={15} /></button>
          <button type="button" aria-label="Fit layout" onClick={() => setZoom(1)} className="rounded-lg p-2 hover:bg-slate-700"><Maximize2 size={15} /></button>
        </div>
      </div>
      <div className="grid xl:grid-cols-[minmax(0,1fr)_230px]">
        <div className="overflow-auto p-3" style={{ maxHeight: 520 }}>
          <svg viewBox={`${-pad} ${-pad} ${length + pad * 2} ${width + pad * 2}`} style={{ width: `${zoom * 100}%`, minWidth: 280 }} className="block">
            <defs>
        ...
        <aside className="space-y-5 border-t border-slate-700 bg-slate-900/60 p-4 xl:border-l xl:border-t-0">
          <div><p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Material utilization</p><p className="mt-1 text-3xl font-semibold text-teal-300">{Number(bed.efficiency_percent || 0).toFixed(1)}<span className="text-lg">%</span></p><p className="mt-1 text-xs text-slate-400">{remainingArea.toFixed(3)} m² remaining in this pull</p></div>
          <div className="border-t border-slate-700 pt-4"><p className="mb-2 flex items-center gap-2 text-xs font-semibold text-white"><MousePointer2 size={13} />{chosen ? `Item #${chosen.item_number}` : 'Inspect a piece'}</p>{chosen ? <div className="space-y-1 text-xs text-slate-300"><p>{chosen.location}</p><p>{mm(chosen.width)} × {mm(chosen.drop)} mm</p><p className="text-slate-400">X {mm(chosen.x_pos_mm)} · Y {mm(chosen.y_pos_mm)}</p></div> : <p className="text-xs leading-relaxed text-slate-400">Select a rectangle to see its size and table position.</p>}</div>
          <div className="border-t border-slate-700 pt-4"><p className="mb-2 text-xs font-semibold text-amber-300">Remaining fabric · {bed.remnants.length} regions</p>{bed.remnants.map((item) => <p key={item.id} className="text-[11px] text-slate-300"><span className="mr-2 text-amber-300">R{item.id}</span>{mm(item.width)} × {mm(item.drop)} mm</p>)}</div>
        </aside>
      </div>
    </div>
  );
